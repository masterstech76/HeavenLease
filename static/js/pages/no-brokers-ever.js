/* ============================================================
 * HeavenLease — No Brokers Ever module
 * In-page chat preview of the owner inbox with backend
 * conversations + WebSocket live updates, gated by the
 * Access Pass. Extracted from no-brokers-ever.html.
 * FIX in extraction: original inline referenced `isPaid`
 * without declaring it (ReferenceError → chat locked open /
 * auto-open dead). Now resolved via api.getSubscription().
...
(function () {
    ...
    function appendIncoming(msg) {
        ...
        saveConversations();
        if (activeConversation && String(activeConversation) === String(msg.conversationId)) renderMessages(activeConversation);
        renderConversations();
    }
    ...
    function renderConversations(filter) {
        ...
        if (filtered.length === 0) {
            items.innerHTML = '<div style="text-align:center;padding:48px 20px;color:#98a2b1"><i class="fas fa-comments" style="font-size:34px;margin-bottom:12px;display:block"></i><p style="font-size:12px;margin:0">No conversations found.</p><a href="properties" style="color:#5b5ce2;font-weight:700;font-size:12px;display:inline-block;margin-top:12px">Browse properties →</a></div>';
            return;
        }
        items.innerHTML = filtered.map((c) => `
            <div class="conversation-item ${activeConversation === c.id ? 'active' : ''}" onclick="openConversation('${c.id}')">
                <div class="conversation-avatar"><i class="fas fa-user"></i></div>
                <div class="conversation-info">
                    <div class="name">${escapeHtmlSafe(c.ownerName)}<span class="time">${escapeHtmlSafe(c.messages && c.messages.length ? c.messages[c.messages.length - 1].time : (c.lastTime || ''))}</span></div>
                    <span class="property-tag"><i class="fas fa-home"></i> ${escapeHtmlSafe(c.property || 'Messages')}</span>
                    <div class="preview">${escapeHtmlSafe(c.messages && c.messages.length ? c.messages[c.messages.length - 1].text : (c.lastMessage || 'Start a conversation'))}</div>
                </div>
    ...
    window.openConversation = function (convId) {
        if (!isPaid) { window.location.href = 'payment?redirect=messages'; return; }
    ...
    function renderMessages(convId) {
        ...
        messagesDiv.innerHTML = Object.keys(grouped).map((day) => `
            <div class="chat-date-divider">${escapeHtmlSafe(day)}</div>
            ${grouped[day].map((m) => `
                <div class="chat-message ${m.from === 'me' ? 'sent' : 'received'}">
                    ${escapeHtmlSafe(m.text)}
                    <span class="msg-time">${escapeHtmlSafe(m.time)}</span>
                </div>
            `).join('')}
        `).join('');
...
async function loadInitial() {
    ...
    /* ===== Access-Pass gate (same as messages.js) ===== */
    (async () => {
        try {
